...
BorhanUrlTokenMapper = {
	
	// ability to format expressions
	formatFunctions : {
		timeFormat : function(value) {
			return BorhanUtils.seconds2npt(parseFloat(value));
		},
	...
	mapDynamicTokens: function(request, cachedUrl, playerConfig){
		...
		for(var attributeName in queryString){
			var attributeValue = BorhanUrlTokenMapper.evaluate(data, queryString[attributeName]);
			BorhanLogger.log("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");		
	...
	mapFixedTokens: function(request, cuePoint, entry, metadata, playerConfig, callback){
		...
		for(var attributeName in queryString){
			var attributeValue = BorhanUrlTokenMapper.evaluate(data, queryString[attributeName], defaultTokenValue);
			BorhanLogger.log("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");
		...
		BorhanLogger.log('Saving cue point url in cache: [' + cachedUrl + ']');
		BorhanCache.set(cuePointUrlKey, cachedUrl, BorhanConfig.config.cache.cuePoint);
	...
	*/
	evaluate : function(data, objectString, defaultTokenValue, limit) {
		...
		limit = limit || 0;
		if (limit > 4) {
			BorhanLogger.log('recursive calls are limited to 5');
			return objectString;
		}
