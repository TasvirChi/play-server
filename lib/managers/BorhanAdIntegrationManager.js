...
require('../utils/BorhanUrlTokenMapper');
require('../adServingProtocols/vast/BorhanVastParser');
var borhanMediaInfo = require('../media/BorhanMediaInfo');
var borhanAspectRatio = require('../media/BorhanAspectRatio');
var parseString = require('xml2js').parseString;

/**
 * @service adIntegration
 */
var BorhanAdIntegrationManager = function(){
	this.initClient(BorhanConfig.config.client);
};
util.inherits(BorhanAdIntegrationManager, borhan.BorhanManager);
 ...
 * @param sessionId
 */
BorhanAdIntegrationManager.prototype.stitch = function(request, response, params){
	...
	var cuePointsKey = BorhanCache.getCuePoints(params.entryId);
	var elapsedTimeKey = BorhanCache.getElapsedTime(params.entryId);
	...
	BorhanCache.getMulti([cuePointsKey, elapsedTimeKey, entryRequiredKey, uiConfConfigKey], function(data){
 ...
 * @param sessionId
 */
BorhanAdIntegrationManager.prototype.stitchCuePoints = function(request, response, params, cuePoints, elapsedTime, encodingIds, uiConfConfig){
	var This = this;
	var encodingIdsKeys = [];
	var mediaInfos = null;
	var entry = null;
	...
	for(var i = 0; i < encodingIds.length; i++){ 
		if(encodingIds[i].trim().length)
			encodingIdsKeys.push(BorhanCache.getMediaInfo(encodingIds[i]));
	}
				
	BorhanCache.getMulti(encodingIdsKeys, function(mediaInfoData){
		mediaInfos = mediaInfoData;
	 	...
	 	if(!cuePoints || !elapsedTime || !encodingIds){
	   	...
	   	var handleCuePoint = function(cuePoint){
	   		...
	   		var cuePointUrlKey = BorhanCache.getCuePointUrl(cuePoint.id);
			var cuePointHandledKey = BorhanCache.getCuePointHandled(cuePoint.id, params.sessionId);
	   		if( (cuePoint.triggeredAt && timeWindowStart < cuePoint.triggeredAt*1000 && cuePoint.triggeredAt*1000 < timeWindowEnd) ||
				...
				BorhanCache.add(cuePointHandledKey, true, BorhanConfig.config.cache.cuePoint, function(){					
					BorhanCache.get(cuePointUrlKey, function(cachedUrl){
						if(cachedUrl){
 ...
 * @param uiConfConfig
 */
BorhanAdIntegrationManager.prototype.stitchAd = function(request, response, partnerId, cachedUrl, cuePoint, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig ) {
	var This = this;
	...
	This.getAdMediaFiles(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig, function(adFileId, adFileInfo){		
		...
		for(var i = 0; i < adFileInfo.mediaInfoIds.length; i++){
			if(adFileInfo.mediaInfoIds[i].trim().length){					
				var encodingId = BorhanCache.getEncodingIdFromMediaInfo(adFileInfo.mediaInfoIds[i]);
	...
	var stitchAdForEncodingId = function(cuePointId, encodingId, adFileId, adFileInfo){	
		...
		var adHandledKey = BorhanCache.getAdHandled(serverAdId);
		BorhanCache.add(adHandledKey, true, BorhanConfig.config.cache.adHandled, function(){
			var stitchParams = {
 ...
 * @param callback
 */
BorhanAdIntegrationManager.prototype.getEntryAndMetadata = function(partnerId, entryId, metadataProfileId, callback){
	var This = this;
	...
	var callMultiRequest = function(){
		...
		if(metadataProfileId){
			var filter = new borhan.client.objects.BorhanMetadataFilter();
			filter.metadataProfileIdEqual = metadataProfileId;
			filter.objectIdEqual = entryId;		
			filter.metadataObjectTypeEqual = borhan.client.enums.BorhanMetadataObjectType.ENTRY;
		...
		This.client.doMultiRequest(function(results){
			...
			if(results && results.length > 0){
				if(results[0].objectType == 'BorhanAPIException'){
					BorhanLogger.error('Client [baseEntry.get][' + results[0].code + ']: ' + results[0].message);
				}					
				else{
					entry = results[0];
				}					
				if(results.length > 1){
					if(results[1].objectType == 'BorhanAPIException'){
						BorhanLogger.error('Client [metadata.list][' + results[1].code + ']: ' + results[1].message);
					}					
						...
						if(metadataStr){
							...
							parseString(metadataStr, function (err, metadata){
								BorhanLogger.debug('Parsed metadata :' + JSON.stringify(metadata));
 ...
 * @returns
 */
BorhanAdIntegrationManager.prototype.getAdMediaFiles = function(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig, downloadCallback, errorCallback) {
	var This = this;	
	...
	var doGetAdMediaFiles = function(evaluatedUrl){
		...
		if(!vastRequestTimeOut){
			vastRequestTimeOut = BorhanConfig.config.adIntegration.vastRequestTimeOut;
		}
		
		BorhanVastParser.parse(evaluatedUrl, headers, vastRequestTimeOut*1000, function(adServerResponse) {
			if (!adServerResponse) {
			...
			var trackingInfo = {};
			var trackingId = BorhanCache.getAdTrackingId(cuePoint.id, sessionId);
			...
			if(ads.length > 0 ){
				...
				BorhanCache.set(trackingId, trackingInfo, BorhanConfig.config.cache.cuePoint);
	...
	var processSelectedAd = function(ad, trackingInfo){
		...
		This.selectMediaFilePerAspectRatio(response, ad, mediaInfos, function(adPerAspectRatio){
			for(var adFileId in adPerAspectRatio){
				...
				adFileInfo.sharedFilePath = BorhanConfig.config.cloud.sharedBasePath + '/ad_download/' + adFileId;
	...
	var roundDuration = function(duration){
		var coefficient = BorhanConfig.config.adIntegration.durationCoefficient;
	...
	var downloadHttpUrl = function(adFileId, adFileInfo, options, successCallback, errorCallback) {
		...
		BorhanUtils.downloadHttpUrl(adFileInfo.fileURL, options, function(filePath){
	...
	var setServerAdIdsInCache = function () {
		...
		for(var adFileId in adFilesInfo){
			...
			for(var i = 0; i < adFileInfo.mediaInfoIds.length; i++){
				var encodingId = BorhanCache.getEncodingIdFromMediaInfo(adFileInfo.mediaInfoIds[i]);
				var serverAdIdKey = BorhanCache.getServerAdId(cuePoint.id, encodingId, sessionId);
				var serverAdId = adFileId + '-' + encodingId;
		...
		for(var serverAdIdKey in serverAdIdValues){
			...
			BorhanCache.set(serverAdIdKey, serverAdIdValues[serverAdIdKey], BorhanConfig.config.cache.adMedia);
		}				
	};
	...
	if(cachedUrl == null){
		BorhanUrlTokenMapper.mapFixedTokens(request, cuePoint, entry, metadata, playerConfig, function(cachedUrl){
			evaluatedUrl = BorhanUrlTokenMapper.mapDynamicTokens(request, cachedUrl, playerConfig);
 ...
 * @returns
 */
BorhanAdIntegrationManager.prototype.selectMediaFilePerAspectRatio = function(response, ad, mediaInfos, callback) {	
	...
	var getAspectRatioGroups = function(){
		...
		for(var mediaInfoId in mediaInfos){
			...
			var group = borhanAspectRatio.convertFrameSize(mediaInfoWidth, mediaInfoHeight);
	...
	var getBestMediaInfo = function(groupMediaInfoIds){
		...
		for (var i=0; i<groupMediaInfoIds.length; i++) {
			...
			if (selectedMediaInfo == null){
				...
				var compare = borhanMediaInfo.compare(selectedMediaInfo, currentMediaInfo);
				if (compare < 0){
					selectedMediaInfo = currentMediaInfo;
				}
			}
	...
	var findBestAdFile = function(adMediaFiles, mediaInfo) {
		...
		var selectAdFilesWithBestAspectRatio = function(){
			...
			for(var i=0; i<adMediaFiles.length;i++){
				...
				var aspectRatio = borhanAspectRatio.convertFrameSize(adMediaFiles[i].width, adMediaFiles[i].height);
			...
			var bestAspectRatio = borhanAspectRatio.convertFrameSizeForAspectRatioKeys(mediaInfoWidth, mediaInfoHeight, aspectRatioKeys);
		...
		for(var i=0; i<bestRatioAdFiles.length; i++){
			...
			if (!adFileCandidate){
				...
				var res = borhanMediaInfo.selectMatchingMediaInfoOption(mediaInfo, option1, option2);
				if(res == 1){
					adFileCandidate = currentAdFile;
				}
			}
	...
	var setAdFileInfo = function(mediaFile, aspectRatioGroup, callback){
		...
		requestPckg.head(mediaFile.fileURL, function (err, res, body) {
			...
			if(parseInt(BorhanConfig.config.adIntegration.calcMd5OnRedirect)){
 ...
 * @param adStart
 */
BorhanAdIntegrationManager.prototype.sendBeacon = function(request, response, params){
	...
	var sendBeaconForType = function(events, eventType){
		var httpGet = function(url, eventType){
			...
			request.setTimeout( BorhanConfig.config.cloud.requestTimeout * 1000, function( ) {});			
	...
	BorhanCache.get(params.trackingId, function(trackingInfos){
		...
		for(var i=0; i < adSequence.length; i++){
			if(trackingInfos && trackingInfos[adSequence[i]]){
				...
				BorhanCache.set(params.trackingId, trackingInfos, BorhanConfig.config.cache.cuePoint);
			}
			else{
...
module.exports.BorhanAdIntegrationManager = BorhanAdIntegrationManager;
