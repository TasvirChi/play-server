...
var BorhanMediaInfo = {
	...
	config: {
		general: {
			bitrate: {
				...
				callback: function(value){
					return BorhanMediaInfo.parseBitrate(value);
				}
			}
		},
		video: {
			duration: {
				names: ['duration'],
				callback: function(value){
					return BorhanMediaInfo.parseDuration(value);
				}
			},
			bitrate: {
			...
			width: {
				names: ['width'],
				callback: function(value){
					return BorhanMediaInfo.parseItem(value, ['pixels']);
				}
			},
			height: {
				names: ['height'],
			...
			frameRate: {
				names: ['frame rate'],
				callback: function(value){
					return BorhanMediaInfo.parseItem(value, ['fps']);
				}
			},
			reframes: {
				names: ['format settings, reframes'],
				callback: function(value){
					return BorhanMediaInfo.parseItem(value, ['frame', 'frames']);
				}
			},
			profile: {
				names: ['format profile'],
				callback: function(value){
					return BorhanMediaInfo.parseVideoProfile(value);
				}
			},			
			colorPrimaries: {
		...
		audio: {
			...
			sampleRate: {
				names: ['sampling rate'],
				callback: function(value){
					return BorhanMediaInfo.parseItem(value, ['khz']) * 1000;
				}
			},
			channels: {
				names: ['channel(s)'],
				callback: function(value){
					return BorhanMediaInfo.parseItem(value, ['channel', 'channels']);
				}
			},
			profile: {
				names: ['format profile'],
				callback: function(value){
					return BorhanMediaInfo.parseAudioProfile(value);
				}
			},
		}		
	},
	...
	parseRaw: function(data, callback){
		...
		for(sectionName in BorhanMediaInfo.config){
			if(!parsedAttributes[sectionName]){
				continue;
			}
			
			mediaInfo[sectionName] = {};
			var section = BorhanMediaInfo.config[sectionName];
	...
	parse: function(filePath, callback){
		...
		fs.stat(filePath, function (err, stats) {
			...
			child_process.exec(cmd, function (error, stdout, stderr) {
				var mediaInfo = BorhanMediaInfo.parseRaw(stdout);
...
module.exports = BorhanMediaInfo;
